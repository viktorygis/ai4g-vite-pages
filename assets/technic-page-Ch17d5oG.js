import{t as e}from"./technics-nqLxC9ju.js";document.addEventListener(`DOMContentLoaded`,async()=>{let t=document.getElementById(`technic-page`);if(!t)return;let n=window.location.pathname.split(`/`).filter(Boolean).pop().replace(`.html`,``);if(!n){i(`Техника не найдена`);return}try{let t=e.find(e=>e.slug===n);if(!t){i(`Техника не найдена`);return}r(t)}catch(e){console.error(e),i(`Ошибка загрузки данных`)}function r(e){document.title=e.title||e.subtitle||`Техника развития`;function n(e){return!e||!e.length?``:`
        <ul>
